...
func TestStart(t *testing.T) {
	...
	container, err := docker.Create(
		...
		"ls",
		[]string{"-al"},
		[]string{path.Join(os.Getenv("PWD"), "docker/images/ubuntu")},
		&Config{
			Ram: 33554432,
		},
	)
	...
	if err := container.Start(); err != nil {
        t.Fatalf("Container couldn't start: %s\n", err)
	}
	container.Wait()
	if container.State.Running {
		t.Errorf("Container shouldn't be running")
...
func TestStop(t *testing.T) {
	...
	if err := container.Start(); err != nil {
        t.Fatalf("Container failed to start: %s\n", err)
	}
	if !container.State.Running {
		t.Errorf("Container should be running")
	}
	if err := container.Stop(); err != nil {
        t.Fatalf("Container failed to stop: %s\n", err)
	}
	if container.State.Running {
		t.Errorf("Container shouldn't be running")
	}
	...
	// Try stopping twice
	if err := container.Stop(); err != nil {
        t.Fatalf("Container failed to stop twice: %s\n", err)
	}
}
