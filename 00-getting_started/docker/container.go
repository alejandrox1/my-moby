...
func createContainer(name string, root string, command string, args []string, layers []string, config *Config) (*Container, error) {
	...
	if err := os.Mkdir(root, 0700); err != nil {
		log.Printf("Error creating root dir for container: %s\n", root)
        return nil, err
	}
...
func (container *Container) Start() error {
	if err := container.Filesystem.Mount(); err != nil {
        log.Printf("Error mounting container's filesystem: %s\n", err)
        return err
	}
    log.Printf("Mounted container's filesystem\n\n\n")
